...
-- Get the storage root from UCI (selected disk mount point)
-- Config uses anonymous sections of type "cloudreve"
...
function auto_get_arch()
    ...
    elseif arch:match("^armv[5-8]") or arch:match("^arm") then
        ...
        local sub = ver or "7"
        target = "armv" .. sub
        if is_hf then target = target end
...
function download_install(url)
    ...
    local app_dir = ensure_app_dir()
    if not app_dir then
        return { code = 1, error = translate('无法在所选磁盘上创建 Configs 目录，请确认硬盘已挂载且可写。') }
    end
